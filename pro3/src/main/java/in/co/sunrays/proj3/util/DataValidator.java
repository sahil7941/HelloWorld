...
public class DataValidator {
	 ...
	 * 
	 * if (isNotNull(val)) { try { return val.matches(pwd); } catch
	...
	/*
	 * public static boolean isMobileNo(String val) { String mobileNo =
	 * "^[7-9][0-9]{9}$"; if (isNotNull(val)) { try { return
	 * val.matches(mobileNo); } catch (NumberFormatException e) { return false;
	 * } } else { return false; } }
	 */
	...
	public static boolean isValidName(String name) {
		...
		if (name == null || name.trim().length() == 0 || name == "") {
			valid = false;
		} else if (!name.trim().matches(upperCaseChars)) {
			valid = false;
	...
	/**
	 * Checks if value is a valid Description
	 * 
	 * @param name
	 * @return boolean
	 */
	public static boolean isValidDescription(String name) {
		boolean valid = true;
		String upperCaseChars = "[a-zA-Z0-9 .]*";
		...
		if (name == null || name.trim().length() == 0 || name == "") {
			...
			valid = false;
		} else if (name.trim().matches(specialChars)) {
			valid = false;
		}
		return valid;
	}
