			...
			<div class="card">
					...
					<div class="modal-content" style="margin-bottom: -9px;">
						...
						<div class="modal-body" style="margin-bottom: -15px;">
							...
							<div class="modal-header justify-content-center bg-dark">
								<h4 class="modal-title " id="ModalTitle">Faculty List
								</h4>
							</div>
							...
							<div class="row" style="margin-top: 10px;">
								...
								<div class="col-md-3">
									<div class="form-group">
										...
										<%
											String err1 = ServletUtility.getErrorMessage("collegeId", request);

											List clist = (List) request.getAttribute("collegeList");
										%>
										<abbr title="Select College Name"> <%=HTMLUtility.getList("collegeIdN", String.valueOf(dto.getCollegeId()), clist, err1)%>
										</abbr>
									</div>
								</div>
							...
							<%-- 	<div class="col-md-2">
									<div class="form-group">
										...
										<input type="text"
											class="<%=(ServletUtility.getErrorMessage("email", request).equalsIgnoreCase("")) ? "form-control"
                ...
                <input type="Date" class="form-control" placeholder="" >
								...
								<div class="col-md-1">
									<div class="form-group">
										<input type="submit"
											...
											value="<%=FacultyListCtl.OP_SEARCH%>" name="operation">
									</div>
								</div>
								<div class="col-md-1">
									<div class="form-group">
										<input type="submit" class="form-control text-white btn btn-info btn-block"
											value="<%=FacultyListCtl.OP_RESET%>" name="operation" />
									</div>
								</div>
								<div class="col-md-3">
								<ul class="pagination float-right">
									<div class="row">
									<li class="page-item">
										<%-- <input type="submit" class="btn page-link text-white bg-primary"  value="<%=FacultyListCtl.OP_PREVIOUS%>" name="operation" <%=(pageNo==1)? "disabled" : "" %>> --%>
										<input type="submit"
										class="<%=(pageNo == 1) ? "btn page-link text-secondary no-drop" : "btn page-link text-white bg-secondary"%>"
										value="<%=FacultyListCtl.OP_PREVIOUS%>" name="operation"
										<%=(pageNo == 1) ? "disabled" : ""%>>
									</li>
                ...
                %>
					...
					: "btn page-link text-white bg-secondary"%>"
										value="<%=FacultyListCtl.OP_NEXT%>" name="operation"
				...
				<div class="card-body justify-content-center text-left"
					...
					: "text-center text-danger"%>"  style="margin-bottom:25px;"><%=onRecord%></h3>
							...
							<%-- 		<input type="text"
										class="<%=(ServletUtility.getErrorMessage("onerecord", request).equalsIgnoreCase(""))
					? "form-control text-center text-success font-weight-bold"
					: "form-control text-center text-danger font-weight-bold"%>"
					...
					<table class="table table-responsive table-bordered"
						...
						<tbody>
							...
							<%
								...
								while (it.hasNext()) {
									FacultyDTO rdto = (FacultyDTO) it.next();
							%>
							...
							<tr class="bg-light text-dark">
								...
								<td width="15%" class="mouse"><%=rdto.getSubjectName()%></td>
								<td width="10%" class="mouse"><%=rdto.getFirstName()%></td>
								<td width="20%" class="mouse"><%=rdto.getLastName()%></td>
								<td width="20%" class="mouse"><%=rdto.getEmailId()%></td>
								<td width="20%"><a href="FacultyCtl?id=<%=rdto.getId()%>"
	...
	<div class="col-md-1">
								<div class="form-group">
									<input type="submit" class="form-control text-white btn btn-primary btn-block"
										value="<%=FacultyListCtl.OP_NEW%>" name="operation" />
								</div>
							</div>
							...
							<div class="col-md-1">
								<div class="form-group">
									<input type="submit" class="<%=(list.size()==0)?"form-control bg-danger text-white no-drop" :"form-control text-white btn btn-danger btn-block"%>"
										value="<%=FacultyListCtl.OP_DELETE%>" name="operation"
...
<div class="row justify-content-center" style="margin-top: 10px;">
	<div class="col-md-1">
	<div class="form-group">
    <input type="submit" class="form-control text-white btn btn-secondary btn-block" value="<%=FacultyListCtl.OP_BACK%>" name="operation" />
    </div>
	</div></div>
<%} %>
