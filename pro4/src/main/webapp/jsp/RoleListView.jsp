...
<body>
	...
	<center>
		...
		<form action="<%=ORSView.ROLE_LIST_CTL%>" name="frm" method="post">
			...
			<table width="100%">
				<tr>
					<td align="center"><label><b>Name :</b> </label><%=HTMLUtility.getList("roleId", String.valueOf(bean.getId()), rolelist, "Role")%>
						&emsp;<label><b>Description :</b></label> <input
						style="width: 200px; padding: 5px;" type="text" name="description"
						placeholder="Enter Name"
						value="<%=ServletUtility.getParameter("description", request)%>">
						&emsp; <input style="padding: 5px; width: 100px;"
			...
			<table border="1" width="100%">
				...
				<tr align="center">
					<td><input type="checkbox" id="sid" name="chk_1"
						onchange="check()" <%if (rBean.getId() == RoleBean.ADMIN) {%>
						value=" " <%} else {%> value="<%=rBean.getId()%>" <%}%>
						<%=(rBean.getId() == RoleBean.ADMIN) ?  "disabled" : ""%>></td>
					<td><%=index++%></td>
					<td><%=rBean.getId()%></td>
					<td><%=rBean.getName()%></td>
